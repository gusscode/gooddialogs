...
export class GoodDialogs {
  ...
  constructor(options?: Options) {
    this.options = {
      ...
      confirmButttonClass:
        options?.confirmButttonClass ??
        "gooddialog-confirm-button gooddialog-button-event",
      cancelButtonClass:
        options?.cancelButtonClass ??
        "gooddialog-cancel-button gooddialog-button-event",
      footerClass: options?.footerClass ?? "gooddialog-footer",
      divIconClass:
        options?.divIconClass ??
        "gooddialog-alert-div-icon gooddialog-svg-container",
  ...
  createComponent = (options?: Options) => {
    const optionsAlert: Options = {
      ...
      confirmButttonClass:
        options?.confirmButttonClass ?? this.options.confirmButttonClass,
      cancelButtonClass:
        options?.cancelButtonClass ?? this.options.cancelButtonClass,
      confirmButtonText:
        options?.confirmButtonText ?? this.options.confirmButtonText,
      cancelButtonText:
        options?.cancelButtonText ?? this.options.cancelButtonText,
    ...
    container.classList.value =
      optionsAlert.containerClass! +
      " " +
      optionsAlert.theme +
      " " +
      (optionsAlert.darkForce ? "dark" : "") +
      " " +
      (optionsAlert.lightForce ? "light" : "");
  ...
  confirm = async (
    message: string,
    options?: Options,
    onResponseCB?: (data: HTMLButtonElement) => void
  ): Promise<boolean> => {
    const {
      alert,
      cancelButton,
  ...
  async withOptions<T>(
    message: string,
    optionsButtons: ButtonOptions<T>[],
    options?: Options
  ): Promise<T> {
    const {
      alert,
    ...
    return new Promise<T>((resolve) => {
      ...
      optionsButtons.forEach((option) => {
        ...
        if (option.buttonClass) {
          ...
          buttonOpt.classList.value =
            options?.confirmButttonClass ?? "gooddialog-confirm-button";
        }
  ...
  createNotification(message?: string, options?: Options) {
    ...
    let container = document.getElementById(
      "gooddialog-notification-container-" + optionsNotification.position
    );
    ...
    notification.classList.value =
      ...
      " " +
      optionsNotification.theme! +
      " " +
      "goodalert-animation-aparecer" +
      " " +
      (optionsNotification.darkForce ? "dark" : "") +
      " " +
      (optionsNotification.lightForce ? "light" : "");
  ...
  form = async <T>(
    ...
    data.forEach((item, index) => {
      ...
      itemLabel.setAttribute(
        "for",
        "gooddialogs-item-" + index + "-" + item.type
      );
    ...
    return new Promise((resolve) => {
      ...
      confirmButton.addEventListener("click", () => {
        const inputsFormElements = document.querySelectorAll(
          ".gooddialogs-input-form-instance"
        );
...
class GooddialogElement extends HTMLElement {
  ...
  connectedCallback() {
    const { container, confirmButton } = goodDialogs.createComponent({
      instance: false,
      persistent: true,
    });
